#!/bin/bash
BASE_DIR=/data/ib-a100-cluster-a-pri-lmt_967/users/kaya/workspace/lmalign/nemo-skills-eval/results/GRPO/IF_open_instruct_check_verify_new
for step in $(seq 250 250 2250)
do
  echo "Summarizing results for checkpoint-${step}"
  results_dir=${BASE_DIR}/checkpoint-${step}/eval-results
...
python3 /data/ib-a100-cluster-a-pri-lmt_967/users/kaya/workspace/lmalign/nemo-skills-eval/scripts/json_to_csv.py --batch ${BASE_DIR} ${BASE_DIR}/combined_metrics.csv
